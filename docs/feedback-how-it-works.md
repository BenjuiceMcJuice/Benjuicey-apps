...
- **Per-app extra notifiers (e.g. Heather for What a Disaster).** Some apps have a second person who should get the feedback. Set that app's env var — for What a Disaster it's `WDA_NOTIFY_EMAIL`, stored as a **Cloudflare secret** (`wrangler secret put WDA_NOTIFY_EMAIL`) so the address is never committed — and the Worker **BCC**s them on the admin notification. BCC (not CC) means they receive the email but their address is **never shown in the header** to Ben or anyone else — it stays private. A "🔔 *Heather* has also been notified of this What a Disaster feedback" line (first name only, no email) makes it visible they're in the loop, and the submitter's confirmation email carries the same note. Only What a Disaster feedback triggers this; leave the var empty to notify only Ben. Sending is from the verified `feedback@whatadisaster.uk` domain, so mail delivers to everyone (not just the Resend account owner).
